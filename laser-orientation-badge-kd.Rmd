...
-   this looks like a student-level dataset of science grades from 2016
    (guess based on semester information and course id)

-   I wonder if there is a more detailed codebook to explain the
    variables - their source, context, how they were collected, what
    grade level(s) are included, why there appears to be variation in
    the total points possible among students in the same course; and
    what q1-10, int, pc, uv mean. I also wonder if students are in the
    file more than once (e.g., is the course a semester or full year
    course? and/or did students take more than one science course?)
...
-   based on what I'm able to see, it looks like rows may represent
    student-course-semester grades

What columns typically (or you think may) represent:

-   columns may represent variables, or the items that may vary across
    student-course-semesters (e.g,. grades) or may not vary (e.g.,
    gender)
...
```{r}
d |> 
  select(student_id, total_points_possible, total_points_earned, percentage_earned, FinalGradeCEMS)
```
...
-   203 students are displayed when filtering to 90% or higher, compared
    to 438 with a cutoff of 70, which makes sense because this should be
    a subset of the group of students displayed at 70 or higher
...
```{r}
d |>
  select(student_id,subject,FinalGradeCEMS,TimeSpent) |> filter(FinalGradeCEMS>=90 | FinalGradeCEMS<=70) |> arrange(TimeSpent)
```
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + subject + int + Gender, data = d)
summary(m2)
```
